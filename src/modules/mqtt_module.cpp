...
static bool _reconnect() {
    if (!_mqttClient.connected()) {
        ...
        String helloTopic = _baseTopic + "/status";
        String helloMsg = _deviceId + " Online";

        if (_mqttClient.connect(_clientName.c_str(), helloTopic.c_str(), 0, true, "Offline")) {
            Serial.println("connected!");
            // Publish hello message
            _mqttClient.publish(helloTopic.c_str(), helloMsg.c_str(), true);
